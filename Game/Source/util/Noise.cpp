...
float Noise::octaves(float x, float y, float z, unsigned int octaves, float frequency, float persistance) const
{
	float v = 0.0f;
	float currentAmplitude = 1.0f;
	for (unsigned int i = 0; i < octaves; i++) {
		v += noise(x * frequency, y * frequency, z * frequency) / currentAmplitude;
...
float Noise::octavesAbs(float x, float y, float z, unsigned int octaves, float frequency, float persistance) const
{
	float v = 0.0f;
	float currentAmplitude = 1.0f;
	for (unsigned int i = 0; i < octaves; i++) {
		v += fabsf(noise(x * frequency, y * frequency, z * frequency)) * currentAmplitude;
		currentAmplitude *= persistance;
		frequency *= 2.0f;
	}
	return v;
